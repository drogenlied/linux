...
static int gpio_pmodoled_spi_probe(struct spi_device *spi)
{
	...
	if (gpio_pmodoled_dev_id == 0) {
		...
		if (status) {
			dev_err(&spi->dev, "Character device region not allocated correctly: %d\n", status);
	...
	gpio_pmodoled_dev->spi = spi;
	pr_info(DRIVER_NAME " [%s] SPI Probing\n", gpio_pmodoled_dev->name);

#ifdef CONFIG_PMODS_DEBUG
	pr_info(DRIVER_NAME " [%s] spi_probe: setup char device\n", gpio_pmodoled_dev->name);
#endif
	 ...
	 */
#ifdef CONFIG_PMODS_DEBUG
	pr_info(DRIVER_NAME " [%s] spi_probe: initialize device\n", gpio_pmodoled_dev->name);
#endif
...
static int gpio_pmodoled_spi_remove(struct spi_device *spi)
{
	...
	gpio_pmodoled_dev = spi_get_drvdata(spi);

	if (np->data == NULL) {
		dev_err(&spi->dev, "pmodoled %s: ERROR: No gpio_pmodoled_device structure found!\n", np->name);
		return -ENOSYS;
	}

#ifdef CONFIG_PMODS_DEBUG
	pr_info(DRIVER_NAME " [%s] spi_remove: Clearing Display\n", gpio_pmodoled_dev->name);
#endif

	/* Clear Display */
	memset(gpio_pmodoled_dev->disp_buf, 0, DISPLAY_BUF_SZ);
	status = screen_buf_to_display(gpio_pmodoled_dev->disp_buf, gpio_pmodoled_dev);
	...
	/* Turn off VCC (VBAT) */
	gpio_set_value(gpio_pmodoled_dev->iVBAT, 1);
	msleep(100);
	/* TUrn off VDD Power */
	gpio_set_value(gpio_pmodoled_dev->iVDD, 1);
#ifdef CONFIG_PMODS_DEBUG
	pr_info(DRIVER_NAME " [%s] spi_remove: Free GPIOs\n", gpio_pmodoled_dev->name);
#endif

	{
		struct gpio gpio_pmodoled_ctrl[] = {
			{ gpio_pmodoled_dev->iVBAT, GPIOF_OUT_INIT_HIGH, "OLED VBat"	},
			{ gpio_pmodoled_dev->iVDD,  GPIOF_OUT_INIT_HIGH, "OLED VDD"	},
			{ gpio_pmodoled_dev->iRES,  GPIOF_OUT_INIT_HIGH, "OLED_RESET" },
			{ gpio_pmodoled_dev->iDC,   GPIOF_OUT_INIT_HIGH, "OLED_D/C"	},
		};
	...
	if (&gpio_pmodoled_dev->cdev) {
#ifdef CONFIG_PMODS_DEBUG
		pr_info(DRIVER_NAME " [%s] Destroy Char Device\n", gpio_pmodoled_dev->name);
#endif
		device_destroy(gpio_pmodoled_class, gpio_pmodoled_dev->dev_id);
		cdev_del(&gpio_pmodoled_dev->cdev);
	}
...
static struct spi_driver gpio_pmodoled_driver = {
	.driver		= {
		.name	= DRIVER_NAME,
...
module_spi_driver(gpio_pmodoled_driver);
